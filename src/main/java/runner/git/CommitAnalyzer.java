...
public class CommitAnalyzer implements Runnable {
    ...
    private final DifferFactory factory;
    ...
    private FileChangeSummary createDummySummary(DiffEntry diff, int errorCode, long totalTime) {
        ...
        DifferMetrics metrics = new DifferMetrics();
    ...
    private FileChangeSummary processDiff(DiffEntry diff) {
        ...
        try {
            ...
            AbstractJavaChangeDiffer differ = factory.createDiffer();

            try {
                differ.diff(srcString, dstString);
                summary.setChanges(differ.getCodeChanges());
                summary.setMetrics(differ.getMetrics());
